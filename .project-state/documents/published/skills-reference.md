...
### `project-doc-suite-generator` (NEW v2.0.1)
Generate a complete baseline report bundle of styled `.docx` and `.xlsx` files from `.project-state/`. Produces: index, tracker workbook (8 sheets with Gantt, dashboard, milestones, deliverables, risks, phases, dependencies, legend), project plan, risk register, milestone specs, architecture overview, and roadmap/KPIs. Triggered by phase transitions, milestone completions, or on demand. Uses `python-docx` and `openpyxl` for rendering.
- **Reads:** manifest, state.json, milestones/, risks/, phases/, decisions/, reporting-matrix.yaml
- **Writes:** reports/baseline/Baseline-Reports-YYYY-MM-DD/, website/public/downloads/baseline/
- **Calls:** project-state (data source)
- **Called by:** project-orchestrator (`baseline` routine), project-phase-gate (post-transition), user (manual)
- **Triggers on:** "generate baseline reports", "create the report bundle", "build the tracker xlsx", "baseline report bundle"
